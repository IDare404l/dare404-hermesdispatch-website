# Hermes Dispatch Tool — Miner Pivot Calculator (GPU Data)
# Source: https://hermesdispatch.dev/tools/miner-pivot-calculator/
# Description: Look up GPU specs from the embedded daily snapshot for the Miner Pivot Calculator.
...
#
# NOTE:
#   This is a read-only data snapshot refreshed daily when hermesdispatch.dev rebuilds.
#   For live crypto prices, use miner_pivot_calculator.py in live mode with your own API key.
#
# MANUAL REGISTRY:
    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                "gpu_slug": {
                    "type": "string",
                    "description": "GPU slug to look up, e.g. nvidia-geforce-rtx-4090."
                }
            },
            "required": ["gpu_slug"]
        }
...
def lookup(args):
    ...
    for slug, info in gpus.items():
        if slug.lower() == gpu_slug or info.get("name", "").lower() == gpu_slug:
            gpu = dict(info)
            gpu["slug"] = slug
            return _ok({"gpu": gpu, "last_updated": SNAPSHOT["last_updated"]})
